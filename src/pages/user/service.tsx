...
import { ChevronDown, ChevronRight, Clock, Loader2, MessageCircle, Paperclip, Plus, Send, Ticket, X } from 'lucide-react';
...
export function ServicePage() {
  ...
  async function loadTickets() {
    if (!user) return;
    try { setTickets(await fetchUserTickets(user.id)); } catch (error) { toast.error('Unable to load support tickets', { description: error instanceof Error && import.meta.env.DEV ? error.message : 'Please try again.' }); } finally { setLoading(false); }
  }
  ...
  async function submitTicket(event: FormEvent) {
    ...
    } catch (error) { toast.error('Could not submit ticket', { description: error instanceof Error && import.meta.env.DEV ? error.message : 'Please try again.' }); } finally { setSubmitting(false); }
  }
  ...
  return <div className="space-y-6">
    ...
    {loading ? <div className="flex h-40 items-center justify-center"><Loader2 className="size-6 animate-spin text-muted-foreground" /></div> : tickets.length === 0 ? <NexCard className="p-8"><EmptyState icon={MessageCircle} title="No tickets yet" description="Create a support ticket and our team will respond here." /></NexCard> : <div className="space-y-3">{tickets.map((ticket) => <TicketThread key={ticket.id} ticket={ticket} onChanged={loadTickets} />)}</div>}
  </div>;
}

function TicketThread({ ticket, onChanged }: { ticket: SupportTicketRow; onChanged: () => Promise<void> }) {
